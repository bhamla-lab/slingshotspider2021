...
Kt=1.25*Kt;
% Forces
Fr1x=-0.075*0.5.*Kr.*delta_lr1.*cosphir1.*heaviside(lr1-pars.lre);
Fr1y=-0.5.*Kr.*delta_lr1.*sinphir1.*heaviside(lr1-pars.lre);

Fr2x=0.075*0.5.*Kr.*delta_lr2.*cosphir2.*heaviside(lr2-pars.lre);
Fr2y=-0.5.*Kr.*delta_lr2.*sinphir2.*heaviside(lr2-pars.lre);

%tension
Ftx=0.075*Kt.*delta_lt.*sinphit.*heaviside(lt-pars.lte);
Fty=-Kt.*delta_lt.*cosphit.*heaviside(lt-pars.lte);
%drag
Cwx=-sinphir1*Cw.*vxs+Cs.*vxs.^2;
Cwy=-0.95*Cw.*vys+Cs.*vys.^2;
...
% 
% filename=sprintf('C_%.02f_x015mm.mat',100);
...
Cwx=0.05*(Cw*xdot+Cs*xdot^2);
Cwy=1*1*Cw*ydot+Cs*ydot^2;
